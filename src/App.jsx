...
const DEFAULT_ROOMS = [
  { id: 1, number: '101', type: 'Superior', price: 275000 },
  { id: 2, number: '102', type: 'Superior', price: 275000 },
  { id: 3, number: '103', type: 'Superior', price: 275000 },
  { id: 4, number: '201', type: 'Superior', price: 275000 },
  { id: 5, number: '202', type: 'Superior', price: 275000 },
  { id: 6, number: '203', type: 'Family', price: 375000 },
  { id: 7, number: '205', type: 'Family', price: 375000 },
  { id: 8, number: '206', type: 'Deluxe', price: 325000 },
  { id: 9, number: '207', type: 'Deluxe', price: 325000 },
  { id: 10, number: '208', type: 'Deluxe', price: 325000 },
  { id: 11, number: '209', type: 'Deluxe', price: 325000 },
  { id: 12, number: '210', type: 'Deluxe', price: 325000 },
  ...
  { id: 16, number: '215', type: 'Deluxe', price: 325000 },
  { id: 17, number: '216', type: 'Deluxe', price: 325000 },
  { id: 18, number: '217', type: 'Deluxe', price: 325000 },
  { id: 19, number: '218', type: 'Deluxe', price: 325000 },
  { id: 20, number: '219', type: 'Deluxe', price: 325000 },
  { id: 21, number: '220', type: 'Deluxe', price: 325000 },
  { id: 22, number: '221', type: 'Deluxe', price: 325000 },
  { id: 23, number: '222', type: 'Superior', price: 275000 },
...
const RoomManager = ({ rooms, setRooms, saveToDb, deleteFromDb }) => {
  ...
  const [currentRoom, setCurrentRoom] = useState({ id: null, number: '', type: 'Deluxe', price: '' });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setIsEditing(false);
      setCurrentRoom({ id: null, number: '', type: 'Deluxe', price: '' });
    } catch (err) {
...
const CreateInvoice = ({ rooms, invoiceCount, saveToDb, currentUser }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    checkIn: '', checkOut: '',
    paymentMethod: 'Cash', paymentStatus: 'Lunas'
  });
  const [roomsData, setRoomsData] = useState([
    { roomId: '', extraBed: 0, discount: 0 }
  ]);
  ...
  const calculateSubTotal = (roomData) => {
    const selectedRoom = rooms.find(r => r.id.toString() === roomData.roomId);
    return selectedRoom ? selectedRoom.price * nights : 0;
  };

  const calculateRoomTotal = (roomData) => {
    const sub = calculateSubTotal(roomData);
    return sub + Number(roomData.extraBed) - Number(roomData.discount);
  };

  const grandTotal = roomsData.reduce((acc, curr) => acc + calculateRoomTotal(curr), 0);
  const totalSubTotal = roomsData.reduce((acc, curr) => acc + calculateSubTotal(curr), 0);
  const totalExtraBed = roomsData.reduce((acc, curr) => acc + Number(curr.extraBed), 0);
  const totalDiscount = roomsData.reduce((acc, curr) => acc + Number(curr.discount), 0);

  const handleAddRoom = () => {
    setRoomsData([...roomsData, { roomId: '', extraBed: 0, discount: 0 }]);
  };
  ...
  const handleRoomChange = (index, field, value) => {
    ...
    newRoomsData[index][field] = value;
  ...
  const handleSubmit = async (e) => {
    ...
    // Validasi
    const isValid = roomsData.every(r => r.roomId !== '');
    if (!isValid || !formData.checkIn || !formData.checkOut) {
      setErrorMsg('Harap lengkapi semua data kamar dan tanggal.');
    ...
    const newInvoice = {
      ...
      createdBy: currentUser?.name || 'Resepsionis',
      ...formData,
      nights: nights,

      roomNumber: firstRoom.roomNumber || '',
      roomType: firstRoom.roomType || '',
      roomPrice: firstRoom.roomPrice || 0,
      discount: totalDiscount,
      extraBed: totalExtraBed,
      total: grandTotal,
    ...
    try {
      ...
      setRoomsData([{ roomId: '', extraBed: 0, discount: 0 }]);
    } catch (err) {
      setErrorMsg(err.message);
      setTimeout(() => setErrorMsg(''), 7000);
    }
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden relative">
      ...
      {errorMsg && (
        <div className="fixed bottom-6 right-6 bg-white shadow-2xl rounded-2xl border border-red-100 p-4 flex items-center z-50 animate-fade-in-up">
          ...
          <div className="mr-6">
            <p className="font-bold text-gray-800 text-sm mb-0.5">Gagal Menyimpan</p>
            <p className="text-gray-500 text-xs">{errorMsg}</p>
          </div>
          <button onClick={() => setErrorMsg('')} className="text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100 p-1.5 rounded-full transition-colors">
            <X className="w-4 h-4" />
      ...
      <div className="p-4 sm:p-6 border-b border-gray-100">
        ...
        <p className="text-gray-500 text-xs sm:text-sm mt-1">Lengkapi data tamu dan penyewaan untuk menerbitkan invoice multi-kamar.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-gray-700 border-b pb-2 text-sm sm:text-base">Informasi Tamu & Tagihan</h3>
            <div>
            ...
            <div>
              ...
              <input type="text" className="w-full p-2 border border-gray-300 rounded-md focus:ring-purple-500 focus:border-purple-500 text-sm"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
              <div>
                ...
                <select className="w-full p-2 border border-gray-300 rounded-md focus:ring-purple-500 focus:border-purple-500 text-sm"
                  value={formData.paymentMethod} onChange={e => setFormData({ ...formData, paymentMethod: e.target.value })}
                >
                  <option value="Cash">Cash</option>
                  <option value="Transfer Bank">Transfer Bank</option>
                  <option value="EDC / Kredit">EDC / Kredit</option>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b pb-2">
              <h3 className="font-semibold text-gray-700 text-sm sm:text-base">Detail Kamar & Biaya</h3>
              <button type="button" onClick={handleAddRoom} className="flex items-center text-sm text-white bg-purple-600 hover:bg-purple-700 px-3 py-1.5 rounded transition-colors">
                <Plus className="w-4 h-4 mr-1" /> Tambah Kamar
              </button>
            </div>
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
              {roomsData.map((roomData, index) => {
                const selectedRoom = rooms.find(r => r.id.toString() === roomData.roomId);
                return (
                  <div key={index} className="p-4 border border-purple-100 rounded-lg bg-purple-50/30 relative">
                    {roomsData.length > 1 && (
                      <button type="button" onClick={() => handleRemoveRoom(index)} className="absolute top-2 right-2 text-red-400 hover:text-red-600 bg-white rounded-full shadow-sm p-1">
                        <X className="w-4 h-4" />
                      </button>
                    )}
                    <h4 className="text-xs font-bold text-purple-800 mb-2">Kamar {index + 1}</h4>

                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Pilih Kamar</label>
                        <select required className="w-full p-2 border border-gray-300 rounded-md bg-white focus:ring-purple-500 focus:border-purple-500 text-sm"
                          value={roomData.roomId} onChange={e => handleRoomChange(index, 'roomId', e.target.value)}
                        >
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-gray-700 mb-1">Biaya Extra Bed</label>
                          <input type="number" min="0" className="w-full p-2 border border-gray-300 rounded-md focus:ring-purple-500 focus:border-purple-500 text-sm bg-white"
                            value={roomData.extraBed} onChange={e => handleRoomChange(index, 'extraBed', e.target.value)}
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-gray-700 mb-1">Diskon</label>
                          <input type="number" min="0" className="w-full p-2 border border-gray-300 rounded-md focus:ring-purple-500 focus:border-purple-500 text-sm bg-white"
                            value={roomData.discount} onChange={e => handleRoomChange(index, 'discount', e.target.value)}
                          />
                        </div>
                      </div>

                      {selectedRoom && (
                        <div className="pt-2 mt-2 border-t border-purple-100 flex justify-between items-center">
                          <span className="text-xs text-gray-500">Subtotal (Kamar {index + 1}):</span>
                          <span className="text-sm font-bold text-purple-700">{formatRupiah(calculateRoomTotal(roomData))}</span>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="flex justify-end items-center">
            <div className="w-full md:w-1/2 lg:w-1/3 bg-gray-50 p-4 rounded-xl border border-gray-200">
              <div className="flex justify-between mb-2 text-sm">
                <span className="text-gray-600">Subtotal ({nights} Malam, {roomsData.length} Kamar)</span>
                <span>{formatRupiah(totalSubTotal)}</span>
              </div>
              <div className="flex justify-between mb-2 text-sm">
                <span className="text-gray-600">Total Extra Bed</span>
                <span>{formatRupiah(totalExtraBed)}</span>
              </div>
        ...
        <div className="flex justify-end mt-6">
          <button type="submit" disabled={!formData.checkIn || roomsData.some(r => !r.roomId)}
...
const InvoiceHistory = ({ invoices, onPrint, deleteFromDb, saveToDb, role }) => {
  ...
  const handleVoidInvoice = async (inv) => {
    try {
      const updatedInv = { ...inv, paymentStatus: 'Void' };
      if (saveToDb) {
        await saveToDb('invoices', updatedInv);
        setToastMsg(`Invoice ${inv.invoiceNumber} berhasil di-Void.`);
        setTimeout(() => setToastMsg(''), 5000);
      }
    } catch (err) {
      setErrorMsg(err.message);
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 relative">
      ...
      {toastMsg && (
        <div className="fixed bottom-6 right-6 bg-white shadow-2xl rounded-2xl border border-green-100 p-4 flex items-center z-50 animate-fade-in-up">
          ...
          <div className="mr-6">
            <p className="font-bold text-gray-800 text-sm mb-0.5">Berhasil!</p>
      ...
      {errorMsg && (
        <div className="fixed bottom-6 right-6 bg-white shadow-2xl rounded-2xl border border-red-100 p-4 flex items-center z-50 animate-fade-in-up">
          ...
          <div className="mr-6">
            <p className="font-bold text-gray-800 text-sm mb-0.5">Gagal</p>
            <p className="text-gray-500 text-xs">{errorMsg}</p>
          </div>
          <button onClick={() => setErrorMsg('')} className="text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100 p-1.5 rounded-full transition-colors">
            <X className="w-4 h-4" />
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status / Total</th>
              <th className="px-4 sm:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Aksi</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {displayedInvoices.length === 0 ? (
              ...
              return (
                <tr key={inv.id} className={`${isVoid ? 'bg-gray-50/50' : 'hover:bg-gray-50'} transition-colors`}>
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-[10px] sm:text-xs text-gray-500">{roomDesc}</div>
                  </td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2 mb-1">
                      {isVoid ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-red-100 text-red-800">VOID</span>
                      ) : inv.paymentStatus === 'Belum Lunas' ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-yellow-100 text-yellow-800">BELUM LUNAS</span>
                      ) : (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-green-100 text-green-800">LUNAS</span>
                      )}
                    </div>
                    <div className={`text-xs sm:text-sm font-bold ${isVoid ? 'text-gray-400 line-through' : 'text-gray-800'}`}>
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex justify-end gap-1 sm:gap-2">
                    ...
                    {role === 'admin' && !isVoid && (
                      <button onClick={() => handleVoidInvoice(inv)} className="text-yellow-600 hover:text-yellow-900 bg-yellow-50 p-1.5 sm:px-2 sm:py-1.5 rounded-md inline-flex items-center transition-colors" title="Void Invoice">
                        <X className="w-4 h-4" />
                      </button>
                    )}
                    {role === 'admin' && (
...
const PrintLayout = ({ invoice, onCancel, currentUser }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 print:static print:bg-white print:block overflow-y-auto">
      ...
      <div id="invoice-print-area" className="bg-white w-full max-w-2xl shadow-2xl print:shadow-none p-6 sm:p-10 flex flex-col relative text-sm mx-auto relative">

        {isVoid && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 opacity-20">
            <h1 className="text-9xl font-black text-red-600 transform -rotate-45">VOID</h1>
          </div>
        )}

        <div className="border-b-4 border-purple-600 pb-4 mb-6 flex justify-between items-start z-10 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6 text-sm z-10 relative">
          <div>
            ...
            {invoice.companyName && <p className="text-gray-700 font-semibold mt-0.5">{invoice.companyName}</p>}
          ...
          <div>
            <h3 className="font-bold text-gray-800 border-b border-gray-200 mb-2 pb-1 uppercase text-xs tracking-wider">Detail Pemesanan:</h3>
            <table className="w-full text-sm">
              <tbody>
                <tr><td className="text-gray-600 py-0.5 w-1/2">Check-in</td><td className="font-medium text-right">{formatTanggal(invoice.checkIn)}</td></tr>
                <tr><td className="text-gray-600 py-0.5 w-1/2">Check-out</td><td className="font-medium text-right">{formatTanggal(invoice.checkOut)}</td></tr>
                <tr><td className="text-gray-600 py-0.5">Durasi</td><td className="font-medium text-right">{invoice.nights} Malam</td></tr>
                {invoice.paymentMethod && <tr><td className="text-gray-600 py-0.5">Metode Bayar</td><td className="font-medium text-right">{invoice.paymentMethod}</td></tr>}
                {invoice.paymentStatus && <tr><td className="text-gray-600 py-0.5">Status Pembayaran</td><td className={`font-bold text-right ${isVoid ? 'text-red-600' : invoice.paymentStatus === 'Belum Lunas' ? 'text-yellow-600' : 'text-green-600'}`}>{isVoid ? 'VOID' : invoice.paymentStatus.toUpperCase()}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>

        <div className="overflow-x-auto w-full z-10 relative">
        ...
        <div className="flex justify-end mb-4 z-10 relative">
          <div className="w-full sm:w-1/2">
            <div className="flex justify-between py-2 border-t-2 border-purple-600 font-bold text-[15px] bg-purple-50 px-4">
              ...
              <span className={`${isVoid ? 'text-red-600 line-through' : 'text-purple-900'}`}>{formatRupiah(invoice.total)}</span>
            </div>
          </div>
        </div>

        <div className="flex justify-end mt-4 pt-2 pb-4 z-10 relative">
        ...
        <div className="mt-2 text-center text-[10px] sm:text-xs text-gray-400 border-t pt-2 print:hidden z-10 relative">
          ...
          {visibleMenuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center px-4 py-3 text-sm rounded-xl transition-all duration-200 ${isActive
